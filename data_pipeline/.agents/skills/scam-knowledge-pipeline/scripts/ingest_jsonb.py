...
parser = argparse.ArgumentParser(
    description="Dry-run or upsert validated classified JSONL into staging_documents.raw_json."
)
...
unverified = sorted(
    {
        r.get("source_name", "?")
        for r in records
        if r.get("source_verification_status") != "verified"
    }
)
not_valid = [r for r in records if r.get("validation_status") != "valid"]
if args.apply and unverified:
    raise SystemExit(
        f"refusing --apply for unverified sources: {', '.join(unverified)}"
    )
if args.apply and not_valid:
    raise SystemExit(
        f"refusing --apply for {len(not_valid)} records with validation_status != valid"
    )
    ...
    copy_rows.append(
        {
            ...
            "source_verification_status": r.get(
                "source_verification_status", "needs_probe"
            ),
            "fetched_at": r.get("fetched_at") or "",
            "raw_json": json.dumps(r, ensure_ascii=False, sort_keys=True),
        }
    )
copy_sql = copy_from_rows_sql("tmp_scam_ingest", cols, copy_rows)

fd, sql_name = tempfile.mkstemp(prefix="scam-ingest-", suffix=".sql")
os.close(fd)
