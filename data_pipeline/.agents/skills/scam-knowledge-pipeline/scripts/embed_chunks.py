...
from common import (
    copy_from_rows_sql,
    load_env,
    ensure_vector_extension_available,
    db_args,
    run_psql_file,
)
...
def vector_to_pg(values, dim):
    if len(values) != dim:
        raise SystemExit(
            f"embedding dimension mismatch: expected {dim}, got {len(values)}"
        )
...
def parse_gemini_embedding_response(response, dim):
    ...
    if not isinstance(values, list):
        raise SystemExit(
            f"Gemini embedding response did not include embedding values: {json.dumps(response)[:1000]}"
        )
...
def gemini_embedding_with_curl(text, model, dim, timeout=60):
    ...
    if proc.returncode != 0:
        raise SystemExit(
            f"Gemini embedding request failed via curl: {proc.stderr[:1000]}"
        )
    try:
        response = json.loads(proc.stdout)
    except json.JSONDecodeError:
        raise SystemExit(
            f"Gemini embedding response was not JSON: {proc.stdout[:1000]}"
        )
    if "error" in response:
        raise SystemExit(
            f"Gemini embedding request failed: {json.dumps(response['error'], ensure_ascii=False)[:1000]}"
        )
...
def gemini_embedding(text, model, dim, timeout=60):
    ...
    try:
        ...
        raise SystemExit(
            f"Gemini embedding request failed with HTTP {exc.code}: {body[:1000]}"
        )
...
def build_embedding(text, provider, model, dim):
    ...
    raise SystemExit(
        "unsupported embedding provider; use provider=fake or provider=gemini"
    )
...
parser = argparse.ArgumentParser(
    description="Dry-run or embed pending chunks into pgvector."
)
parser.add_argument("--env-file")
parser.add_argument(
    "--source",
    help="Limit embedding to chunks whose document source_name matches this value.",
)
...
source_filter_d = (
    f" AND d.source_name = '{quote_sql_literal(args.source)}'" if args.source else ""
)
proc = subprocess.run(
    db_args()
    + [
...
embedded_rows = [
    (chunk_id, build_embedding(content, provider, model, dim))
    for chunk_id, content in rows
]

copy_sql = copy_from_rows_sql(
    "tmp_embeddings",
    ["id", "embedding"],
    [{"id": chunk_id, "embedding": vector} for chunk_id, vector in embedded_rows],
)
fd, sql_name = tempfile.mkstemp(prefix="scam-embed-", suffix=".sql")
os.close(fd)
