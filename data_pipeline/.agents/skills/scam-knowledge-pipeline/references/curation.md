...
標題、敘事、`red_flags[].text`、出處、查證題的題目／選項／解析，都會原封不動出現在遊戲畫面上。
2026-09-25 整理題庫時，120 題裡有 80 題用半形逗號、13 處破折號，讀起來就是 AI 寫的，
只好整批回頭修（`deploy/sql/2026-09-25-question-bank-wording.py`）。新題目照下面寫，不要再產生同樣的問題。

驗證器會擋（`scripts/common.py` 的 `WRITING_RULES`，兩個 validate 腳本都會檢查）：

1. **中文旁用全形標點**：`，：；！？（）`。英文與數字之間的半形標點不受影響（例如 `ATM, 165` 不會被擋）。
2. **不用破折號**：`——`、`—`、`--` 都不要。要補充就另起一句，或用括號。
3. **不用 AI 腔的套話**：「關鍵在於」「值得注意的是」「總而言之」「綜上所述」「不難發現」「至關重要」。
4. **不用公文用語**：「予以」「係指」「係屬」「之虞」「爰此」。寫成平常說話的樣子。
5. **出處開頭**：改編自真實案例寫「改編自：」，依官方流程寫的正常案例寫「依據：」，冒號是全形。
6. **選項只寫做法，不寫理由**：選項裡不要有「因為、所以、以免、才能、避免、確保」。
   理由寫在正解時，玩家不看題目也知道選它；理由一律放在解析。

驗證器擋不到、要自己檢查的：

- **少用「不是…而是…」這種句型**，尤其在解析裡。對話裡自然出現可以，但不要拿來當結論句。
- **「此」「該」開頭的指稱**（「此類商品」「該筆金額」）改成「這類」「這筆」。
- **話術標籤要跟句子對得上**：每一條 `red_flags` 先讀句子、再選 tag，不要先決定 tag 再湊句子。
  2026-09-25 發現滑卡有 11 張標錯（句子裡沒有催促，卻標了 `time_pressure`）。對照遊戲畫面上的白話名稱來選：
  ...
  中文名稱以 `backend/app/core/weakness.py` 為準。解析裡要提到話術時，用這些白話名稱，
  不要寫「時間壓力」「權威服從」「社會認同」這類術語。

遊戲端還有一道守門：`backend/tests/unit/test_player_text.py` 會掃 `deploy/seed/*.sql`，
種子檔裡只要有中文旁的半形標點或破折號就會失敗。兩邊的規則要一致，改了一邊記得改另一邊。
