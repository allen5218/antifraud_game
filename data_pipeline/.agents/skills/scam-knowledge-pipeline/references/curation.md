...
同一批 `game_cases` 會產生四種 quiz 題型。前三種直接從 `game_cases` 長出來，
第四種（verification）另外存在 `game_case_questions` 子表，不是從 `red_flags` 生的——
只讀這一節不會知道要出那種題，寫法見下方「怎麼出一題不洩題的查證題」。
策展時必須同時滿足：
...
#### 但「別的情境下的正解」還不夠——三個洩題管道都要一起堵

上面那版探針量到 33.3%，看起來達標，**但人工審閱發現一半的題目讀情境裡的
名詞就能對**：選項分屬不同生活領域（售票／餐廳／報關），看到「包裹」就能刪掉
另外兩個，不必判斷哪個管道適合這個局面。探針抓不到，因為它遮掉情境——
那正是它要測的東西。

修正過程中又踩到兩個新的洩題管道，都是探針抓到的：

| 寫法 | 洩題率 | 為什麼 |
|---|---|---|
| 誘答項來自別的故事 | 33%（探針過） | 但看情境的名詞就能對 |
| 誘答項同情境、但查不同對象 | **100%** | 題幹說「物件本身」，誘答項查人、查交易 → 題幹自己選出答案 |
| 正解一律最全面／最保守 | **53%** | 固定挑最謹慎的那個就會贏 |

**能同時滿足的寫法：同領域叢集。** 把同一類查證情境的題目分成一組，
整組共用一套選項，正解在組內分散：
...
三個選項都是這個領域內真的走得通的管道，所以名詞對號入座失效；
正解隨情境而變，所以不看情境也猜不中。實測 **36.7%（高於基準線 3.3%）**。

檢查清單（出完題自己跑一遍）：

- 同一組選項至少用在 3 題以上，且每個選項當正解的次數不超過總數的一半
- 三個選項指向**同一個對象**（都查物件、或都查管道），不要一個查物件一個查人
- 正解不可以總是最全面、最專業、最謹慎的那一個
- `evidence_scope` 的三個選項要用**同一種句式**。正解寫成「某物列有某事」、
  誘答項寫成「已確認／已受理」的話，固定挑描述物證的那個就會贏

#### 165 什麼時候才是正解

第一版 30 題裡 **165 一次都沒有出現**——不是刻意的，是被自己的規則擠掉的：
`VERIFICATION_WORDS` 把 `"165"` 列為查證字樣，一旦正解含 165 就會觸發
「正解不得是唯一含查證字樣的選項」，最自然的寫法直接被擋。
**優化洩題率的副作用，把台灣最重要的查證管道整個排除在題型外。**

解法不是改規則，是讓整組選項都含查證字樣，並且想清楚 165 的適用時機：

| 情境 | 該走哪條 |
|---|---|
| 對方身分查不出來（沒有名稱的「基金會」、網路上認識的人） | **165** |
| 對方宣稱是你的往來機構（銀行、平台、院所） | **直接找本尊**（卡背客服、官方入口、總機轉窗口） |
| 對方在賣需要資格的商品或服務（投資建議、代客操作） | **主管機關公開名單** |

**能直接找到本尊時，找本尊比查手法更快。** 所以 165 該有題目是正解、
也該有題目是誘答項——只出現在其中一邊，教出來的都是半套。

> 110 目前沒有題目用得上:本題庫的敘事一律停在「還沒付錢」的決策點,
> 沒有已發生的財損。要出 110 的題就得先有停在損失之後的案例。

#### evidence_scope 怎麼寫

問的是「依目前手上的資料，哪一項描述成立」——考的是**推論範圍**，
不是哪個證據比較強。

- 題幹**不要用「最完整」**。用「最完整」會把人推向聽起來查得最徹底的選項，
  但正解常常是「僅取得對方說法」這種最小主張，兩邊互相打架。
- 三個選項都要是**具體的物證描述**，差別在這段敘事裡有沒有真的出現。
  不要拿「敘事明說沒有的東西」當誘答項——那太好刪。
- 解析要說清楚「這份資料能支持到哪裡、不能支持什麼」。
