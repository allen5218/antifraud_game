...
- `allow_unclassified_message_sample`: when true, an unclassified `scam`/`message_sample` remains valid with `taxonomy_code: null`. This is restricted by schema to `tw_cofacts_scam_messages`, whose upstream query already filters the Cofacts scam category and `RUMOR` replies; normalization stores the document without category rows for later manual curation.
