...
- `pagination`: source-specific pagination config. FSC uses `type=post_form`, configurable `page_field`, `first_page`, and bounded `max_pages`; Cofacts follows `pageInfo.lastCursor`, with edge-cursor fallback, and stops at its source-level `max_pages` (default 20).
...
- `headers`, `json`, `form`: endpoint-specific request headers, JSON body, or URL-encoded form body. Cofacts 的 GraphQL query 只可列出 `node.text` 與稽核欄位，不可加入 `articleReplies` 回應內文。
...
- A fetched record is `valid` only when HTTP succeeded, text is non-empty, the response was not truncated, and classification confidence is above zero；唯一例外是明確設定 `allow_unclassified_advisory` 的 advisory，可用 `taxonomy_code: null` 與 confidence 0 保留。
