...
### 全套 Docker 環境
```bash
docker compose watch          # 啟動 db / backend / frontend / adminer / traefik，並熱重載
docker compose logs backend   # 看單一服務 log
```
URL：後端 `:8000`（Swagger 在 `/docs`）、前端 `:5173`、Adminer `:8080`、Traefik `:8090`。
...
### Skills 知識庫機制
`backend/skills/<fraud-type>/` 內的 Markdown（每種詐騙類型一個目錄）由 `pydantic-ai-skills` 的 `discover_skills` / `SkillsToolset` 載入，作為 AI 出題依據。它以唯讀 Docker volume 掛載（`./backend/skills:/app/backend/skills:ro`），**修改後不需重建映像**，只要 `docker compose restart backend`。
...
## CI / 部署
`.github/workflows/` 內：`test-backend.yml`、`test-docker-compose.yml`、`playwright.yml`、`pre-commit.yml` 負責 CI；`deploy-staging.yml`、`deploy-production.yml` 透過 Docker Compose + Traefik 部署到對應子網域。部署細節見 `deployment.md`。
