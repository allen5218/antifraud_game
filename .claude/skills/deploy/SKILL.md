...
逐條**實際操作**，不要只看 HTTP 狀態碼。`quick/swipe/deck`、`pretest/questions`、`mascot/*`、`economy/*` 即使沒灌 `game_cases` 也全部回 200——它們讀的是 `init_db()`（由 `prestart` 呼叫）自動 seed 的 `swipe_card` / `pretest_question` / `mascot_item` / `property_tier`。**只有 quiz 與 scenario 依賴 `game_cases`**，也只有這兩條會暴露內容缺口。
